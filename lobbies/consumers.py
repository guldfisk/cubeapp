...
from draft.game import Draft  # noqa:F401
...
from sealed.game import Sealed  # noqa:F401
...
class LobbyConsumer(AuthenticatedConsumer):
    ...
    def _on_user_authenticated(self, auth_token: t.AnyStr, user: get_user_model()) -> None:
        self._send_message(
            "all_lobbies",
            lobbies=[lobby.serialize_with_key(self.scope["user"]) for lobby in LOBBY_MANAGER.get_lobbies().values()],
        )
