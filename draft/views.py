        ...
        if picks:
            ...
            try:
                previous_pick = models.DraftPick.objects.get(
                    booster_id=current_pick.booster_id, pick_number=current_pick.pick_number - 1
                )
        ...
        return Response(
            {
                ...
                "pool": [
                    [CubeableSerializer.serialize(cubeable, request) for cubeable in pick.pick.added_cubeables]
                    for pick in picks[:-1]
                ],
...
class PickSearch(generics.ListAPIView):
    queryset = (
        models.DraftSession.objects.filter(
