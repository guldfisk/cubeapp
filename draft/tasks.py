...
def clean_drafts() -> None:
    ...
    for draft in (
        models.DraftSession.objects.annotate(
...
def create_draft_session_related_printings(draft_session_id: int) -> None:
    RelatedPrinting.objects.bulk_create(
        ...
        for pick in DraftPick.objects.filter(seat__session_id=draft_session_id)
        for printing in set(Cube(pick.cubeables).all_printings)
    )
