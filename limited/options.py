...
from lobbies.games.options import IntegerOption, Option, OptionsValidationError
...
class CubeReleaseOption(Option[int]):
    ...
    def default(self) -> int:
        return (
            CubeRelease.objects.select_related(
                "versioned_cube",
            )
            .filter(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._default_booster_specification = (
            ...
            if default_booster_specification is None
            else default_booster_specification
        )
        ...
        return [
            {
                ...
                **{
                    key: option.default
                    for key, option in self._allowed_booster_specifications[
                        self._default_booster_specification
                    ].items()
                },
            }
    ...
    def validate(self, value: t.Any) -> PoolSpecificationOptions:
        ...
        for specification in value:
            if not isinstance(specification, t.Mapping):
                raise OptionsValidationError(
                    f"invalid value for {self._name}: specification options must be a mapping"
                )
            ...
            specification_options = {
                ...
                for option, value in specification.items()
