...
class WishList(TimestampedModel, models.Model):
    ...
    def last_updated(self):
        return max(itertools.chain((self.updated_at,), (wish.last_updated for wish in self.wishes.all())))
...
class Wish(TimestampedModel, models.Model):
    ...
    def last_updated(self):
        return max(
            itertools.chain(
                (self.updated_at,), (cardboard_wish.last_updated for cardboard_wish in self.cardboard_wishes.all())
            )
        )
...
class IsFoil(Requirement):
    ...
    @classmethod
    def deserialize(cls, values):
        return cls(is_foil=strtobool(values["is_foil"]))
...
class IsAltered(Requirement):
    ...
    @classmethod
    def deserialize(cls, values):
        return cls(is_altered=strtobool(values["is_altered"]))
...
class IsSigned(Requirement):
    ...
    @classmethod
    def deserialize(cls, values):
        return cls(is_signed=strtobool(values["is_signed"]))
