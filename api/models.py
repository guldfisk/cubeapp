...
class VersionedCube(mixins.SoftDeletionModel, models.Model):
    ...
    def full_release_history(self, before_inclusive: datetime.datetime) -> t.Iterator[CubeRelease]:
        ...
        if self.forked_from_release_id:
            yield from self.forked_from_release.versioned_cube.full_release_history(
                self.forked_from_release.created_at
            )
...
class CubeRelease(models.Model):
    ...
    @classmethod
    def create(
        cls, cube: Cube, versioned_cube: VersionedCube, infinites: Infinites, intended_size: int = 360
    ) -> CubeRelease:
        return cls.objects.create(
            ...
            name=NameGenerator().get_name(
                int(
                    hashlib.sha1(cube.persistent_hash().encode("ASCII")).hexdigest(),
                    16,
                )
            ),
