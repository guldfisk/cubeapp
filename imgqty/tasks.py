...
def check_new_image_records() -> None:
    for draft_session in (
        ...
        .order_by("started_at")
        .prefetch_related("pool_specification__specifications__release")
    ):
        ...
        with transaction.atomic():
            if largest_image_pick:
                ...
                record = models.ImageQtyRecordPack.objects.create(
                    ...
                    probability=ImageQtyProbabilityManager(release.cube, pack_size).probability_at_least_images(
                        largest_image_amount
                    ),
                )
